...
//问题csid=?
	...
	public List<Product> findTop() {
		String hql = "from Product p where p.userId in (from User u where u.vipFlag = '1') and p.is_top = 1";
		hql += " order by p.pdate desc";
		int rows = 10;
		return find(hql,1,rows);
	}
	...
	public List<Product> findByPrice(String min, String max) {
		String hql = "";
		if("".equals(min)) {
			hql = "from Product p where p.cur_price < " + max;
		} else if("".equals(max)) {
			hql = "from Product p where p.cur_price > " + min;
		} else {
			hql = "from Product p where p.cur_price > " + min + "and p.cur_price < " + max;
		}
		return find(hql);
	}
	
	public void topProduct(Integer pid){
		String hql = "update Product p set p.is_top = 1 where pid = " + pid;
