    ...
    <c:if test="${fn:length(sessionScope.cart.cartItems)!=0}">
        <div class="span24">
                    ...
                    <tr>
                        ...
                        <c:if test="${currentTime !=  privilegeTime}">
                            <td>￥<fmt:formatNumber type="number" minFractionDigits="1"
                                                   value="${cartItem.product.cur_price }" maxFractionDigits="1"/>
                            </td>
                            <%--<td class="quantity" width="60"><c:out value="${cartItem.count}" /></td>--%>
                            <%--<td width="140"><span class="subtotal">￥<c:out--%>
                            <%--value="${cartItem.product.cur_price*cartItem.count}" /></span></td>--%>
                        </c:if>

                        <c:if test="${currentTime ==  privilegeTime}">
                            <td>￥<fmt:formatNumber type="number" minFractionDigits="1"
                                                   value="${cartItem.product.cur_price*cartItem.product.categorySecond.category.discount }"
                                                   maxFractionDigits="1"/>
                            </td>
                            <%--<td class="quantity" width="60"><c:out value="${cartItem.count}" /></td>--%>
                            <%--<td width="140"><span class="subtotal">￥<c:out--%>
                            <%--value="${cartItem.product.cur_price*cartItem.product.categorySecond.category.discount*cartItem.count}" /></span></td>--%>
                        </c:if>

                            <%--<td>￥<fmt:formatNumber type="number" minFractionDigits="1" value="${cartItem.product.cur_price*cartItem.product.categorySecond.category.discount }" maxFractionDigits="1"/>--%>
                            <%--</td>--%>
            ...
            <div class="total">
                ...
                <c:if test="${sessionScope.user != null}">
                    ...
                    <c:if test="${currentTime ==  privilegeTime}">
                        ...
                        <%-- 判断购买商品是否满足优惠条件 --%>
                        <%--<c:if test="${cartItem.product.cur_price*cartItem.product.categorySecond.category.discount*count >= ticket.consume}">
            ...
            <div>
            	...
            	<tr>
                        <td width="60">
                            <a href="${pageContext.request.contextPath }/findByPid/<c:out value="${product.pid}"/>">
                                <img style="width: 60px"
                                        src="${pageContext.request.contextPath}/<c:out value="${product.image}"/>"/>
                            </a>
                        </td>
                        <p>￥<c:out value="${product.cur_price}"/>元</p>
                </tr>
            </div>
            <div class="bottom">
